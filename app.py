...
def generate_docx(rca_data, capa_data):
    """Generates a DOCX report with Brafe branding, logo, and color-coded sections."""
    try:
        doc = Document()
        ...
        try:
            ...
            doc.add_paragraph("Brafe Logo Not Found").alignment = WD_ALIGN_PARAGRAPH.CENTER
        # Main title
        title = doc.add_heading('Corrective Action Report (CAR)', 0)
        ...
        add_section_heading('1. Report Details')
        add_key_value_pair('CAR Number', capa_data.get('car_number', 'N/A'), use_accent=True)
        add_key_value_pair('Generated By', rca_data.get('generated_by', 'N/A'))
        add_key_value_pair('Generated Date', datetime.now().strftime('%Y-%m-%d'), use_accent=True)
        ...
        if rca_data.get('record_type') == 'Customer':
            ...
            add_key_value_pair('PO Number', rca_data.get('po_number', 'N/A'))
            add_key_value_pair('Work Order', rca_data.get('work_order', 'N/A'), use_accent=True)
        add_horizontal_line()
        # Section 2: Problem Description
        add_section_heading('2. Problem Description')
        add_body_text(rca_data.get('problem_description', ''))
        add_horizontal_line()
        ...
        if technique == '5 Whys':
            ...
            add_body_text("Pareto analysis identifies the most significant factors in a set of data.")
        ...
        add_section_heading('4. Corrective & Preventive Actions')
        add_key_value_pair('Corrective Action', capa_data.get('corrective_action', ''), use_accent=True)
        add_key_value_pair('Preventive Action', capa_data.get('preventive_action', ''))
        add_key_value_pair('Responsible Person', capa_data.get('responsible', ''), use_accent=True)
        add_key_value_pair('Due Date', capa_data.get('due_date', ''))
        add_key_value_pair('Status', capa_data.get('status', ''), use_accent=True)
        add_horizontal_line()
        # Section 5: Approvals
        add_section_heading('5. Approvals')
        responsible_sig = capa_data.get('responsible_sig', '[Signed by Responsible Person]')
        approver_sig = capa_data.get('approver_sig', '[Signed by QA Approver]')
        p = doc.add_paragraph()
        run = p.add_run(f"Responsible Person: {responsible_sig}")
        run.italic = True
        run.font.size = Pt(11)
        run.font.color.rgb = RGBColor(0, 102, 204)  # Brafe Light Blue
        p = doc.add_paragraph()
        run = p.add_run(f"QA Approval: {approver_sig}")
        run.italic = True
        run.font.size = Pt(11)
        run.font.color.rgb = RGBColor(0, 102, 204)
        add_key_value_pair('Date', datetime.now().strftime('%Y-%m-%d'), use_accent=True)
        add_horizontal_line()
        # Section 6: Evidence Photos
        if rca_data.get('images'):
            add_section_heading('6. Evidence Photos')
            for img_data in rca_data['images']:
                try:
                    ...
                    add_body_text(f"Error processing image: {e}")
            add_horizontal_line()
        # Save to BytesIO
